...
<body>
		...
		<div class="categoryMain-list">
			...
			<ul class="item">
				...
				<li style="width:100px" class="operation-li">
					<input data-pid="<%=p.getPid()%>" class="checkbox-input" name="offsale" type="checkbox"></input>
				</li>
			</ul>
			<%
				}
		...
		$(function() {
			...
			$(".checkbox-input").change(function(){
				if($(".checkbox-input:checked")[0]==undefined){
						...
						$(".operation-title-li").append("<button id='shelves-btn' class='btn off-shelves-btn submit-btn' >下 架</button>");												
				}				
				//点击下架
				$("#shelves-btn").click(function(){
					confirmPullOffShelves();
				});
	...
	<%=pageInfo.getCurrentPage()%>
			...
			$(".search-btn").click(function() {
				assignContent("getOnSaleProductPageWithCondition", 1);
			});
			...
			if (<%=pageInfo.getCurrentPage()%>== 1) {
				...
				$(".first-page-btn").click(function() {
					assignContent("toPullOffShelvesjsp", 1);
				});

				$(".previous-page-btn").click(
						function() {
							assignContent("toPullOffShelvesjsp",
	...
	<%=pageInfo.getNextPage()%>
		...
		function changePage(currentPage) {
			assignContent("toPullOffShelvesjsp", currentPage);
		}

		function confirmPullOffShelves() {
			$.confirm({
				title : ' ',
				content : '确定将已选商品下架吗？',
				confirm : function() {
		...
		function pullOffShelves(pidStr) {
			assignContent("pullOffShelves",
				<%=pageInfo.getCurrentPage()%>
				, pidStr);
		}
